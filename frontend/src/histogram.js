...
const Histogram = () => {
  ...
  const processData = (data) => {
    ...
    const uniqueLabs = new Set();
    const uniqueSchools = new Set();
    const pcCountPerSchool = {};
    let totalUsages = 0;

    data.forEach((item) => {
      const schoolName = item.schoolname;
      const pcnum = item.pcnum;
      const day = moment(item.starttime, 'DD/MM/YYYY, hh:mm:ss a').format('YYYY-MM-DD');
      ...
      if (!pcCountPerSchool[schoolName]) {
        pcCountPerSchool[schoolName] = new Set();
      }
      pcCountPerSchool[schoolName].add(pcnum);
    });
    ...
    const totalUniquePCs = Object.values(pcCountPerSchool).reduce((sum, pcSet) => sum + pcSet.size, 0);

    setActivePCs(totalUniquePCs);
    setActiveLabs(uniqueLabs.size);
    setTotalPCUsages(formatTime(totalUsages));
    setActiveSchools(uniqueSchools.size);
  };
