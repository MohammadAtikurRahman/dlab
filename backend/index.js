...
app.get("/get-pc", async (req, res) => {
  try {
    const pcData = await AllTime.find({}, undefined, { sort: { 'lasttime': -1 } });
    res.json(pcData);
  } catch (err) {
...
app.get("/get-video", async (req, res) => {
  try {
    const videoData = await VideoInfo.find({}, undefined, { sort: { video_end_date_time: -1 } });
    res.json(videoData);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
app.get("/get-interval", async (req, res) => {
  try {
    const videoData = await IntervalInfo.find({}, undefined, { sort: { lasttime: -1 } });
    res.json(videoData);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
