...
// Updated endpoint to use Redis caching and detailed logging
app.get('/get-video', cacheMiddleware('get-video'), async (req, res) => {
  try {
    const videoData = await VideoInfo.find({}).lean().exec();
    console.log('Fetched video data:', videoData.length);
