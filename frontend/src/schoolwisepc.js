...
//                 {Array.from(new Set(schoolData.map(item => `${item.schoolname} (EIIN: ${item.eiin})`))).reverse().map((school, index) => (
...
function SchoolwisePC() {
    ...
    const handleSchoolClick = (schoolName, fromSearch = false) => {
        if (fromSearch) {
            if (selectedSearchSchool && selectedSearchSchool[0].schoolname === schoolName) {
                ...
                const schoolInfo = schoolData.filter(item => item.schoolname === schoolName);
    ...
    return (
        <div className="container mt-5">
            ...
            <h3>All Schools</h3>
            <ul className="list-group">
                {Array.from(new Set(schoolData.map(item => item.schoolname))).reverse().map((school, index) => (
                    <li key={index} className="list-group-item list-group-item-action list-group-item-primary d-flex justify-content-between align-items-center" onClick={() => handleSchoolClick(school)}>
                        {school} (EIIN: {schoolData.find(item => item.schoolname === school).eiin})
                        <button className="btn btn-secondary" onClick={(e) => {
                            e.stopPropagation(); // Prevent li onClick from firing
                            const schoolInfo = schoolData.filter(s => s.schoolname === school);
                            downloadCSV(schoolInfo);
                        }}>Download Info</button>
                    </li>
                ))}
